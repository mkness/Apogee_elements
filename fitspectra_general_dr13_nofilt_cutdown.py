...
normed_training_data = 'normed_dr13_cutdown.pickle'
...
offsets = np.array([mean(rescale(T_est)), mean(rescale(g_est)), mean(rescale(feh_est)),mean(rescale(C)), mean(rescale(N)), mean(rescale(O)), mean(rescale(Na)), mean(rescale(Mg)), mean(rescale(Al)), mean(rescale(Si)), mean(rescale(S)), mean(rescale(K))])
#, mean(rescale(Ca)), mean(rescale(Ti)), mean(rescale(V)), mean(rescale(Mn)),mean(rescale(Ni)), mean(rescale(P)), mean(rescale(Cr)), mean(rescale(Co)), mean(rescale(Cu)), mean(rescale(Rb))]) 
...
if __name__ == "__main__":
    ...
    fpickle2 = "coeffs_dr13_nofilt_cutdown.pickle"
    ...
    if self_flag < 1:
      ...
      for each in bl: 
        ...
        testmetaall, inv_covars = infer_labels_nonlinear("coeffs_dr13_nofilt_cutdown.pickle", testdataall, ids, field+"tags_dr13_nofilt_30e_cutdown.pickle",0.00,1.40) 
    if self_flag == 1:
      field = "self_alpha_"
      file_in = open(normed_training_data, 'r') 
      testdataall, metaall, labels, Ametaall, cluster_name,ids = pickle.load(file_in)
    ...
    if self_flag == 2:
      ...
      file_in.close() 
      testmetaall, inv_covars = infer_labels_nonlinear("coeffs_dr13_nofilt_cutdown.pickle", testdataall, ids, field+"tags_dr13_nofilt_cutdown.pickle",-10.950,10.99) 
    if self_flag == 3:
      leave_one_star_out()
    if self_flag == 4:
      pixlist = loadtxt("pixtest4.txt", usecols = (0,), unpack =1) 
